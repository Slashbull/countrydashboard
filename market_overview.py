...
def detailed_analysis_dashboard(data: pd.DataFrame):
    ...
    with tabs[0]:
        ...
        periods = list(data["Period"].cat.categories) if hasattr(data["Period"], "cat") else data["Period"].unique().tolist()
        if len(periods) >= 2:
        ...
        partner_vol = data.groupby("Partner")["Tons"].sum().reset_index().sort_values("Tons", ascending=False)
        ...
        st.write(f"**Top Partner:** {top_partner} accounts for **{top_partner_share:,.2f}%** of total volume.")
    ...
    with tabs[1]:
        ...
        partner_breakdown = data.groupby("Partner")["Tons"].sum().reset_index().sort_values("Tons", ascending=False)
        fig_bar = px.bar(
            partner_breakdown,
            x="Partner",
            y="Tons",
    ...
    with tabs[2]:
        ...
        detail_data = data[data["Partner"] == selected_partner]
        ...
        # Add annotations for peak month (if any)
        ...
        if "Year" in detail_data.columns:
...
# For testing purposes only – remove or replace with actual data in production.
if __name__ == "__main__":
    # Create an empty DataFrame (this block is for development testing)
    sample_data = pd.DataFrame()
    detailed_analysis_dashboard(sample_data)
