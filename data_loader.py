...
def upload_data() -> pd.DataFrame:
    ...
    Converts 'Year' and 'Month' columns into a combined 'Date' column.
    Returns the processed DataFrame.
    """
    st.markdown("<h2 style='text-align: center;'>📂 Upload or Link Data</h2>", unsafe_allow_html=True)
    
    # Check if data is already in session (handled in core_system.py)
    ...
    if df is not None and not df.empty:
        # Convert Year and Month columns (if present) and create a Date column
